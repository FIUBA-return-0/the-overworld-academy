...
const validateStudentId = async (req, res, next) => {
  ...
  if (alumno !== undefined && (alumno <= 0 || isNaN(alumno))) {
    return res.status(400).json({
      error: "El id de alumno enviado debe ser un número mayor que 0",
    });
  }
  next();
};
...
const validateSubjectId = async (req, res, next) => {
  ...
  if (materia !== undefined && (materia <= 0 || isNaN(materia))) {
    return res.status(400).json({
      error: "El id de materia enviado debe ser un número mayor que 0",
    });
  }
  next();
};
...
const validateQueryParamsG = async (req, res, next) => {
  ...
  if (!alumno && !materia) {
    return res
      .status(400)
      .json({ error: "debe enviar un alumno o una materia" });
  }
