...
const getSubject = async (data) => {
  ...
  const query = `
    select m.id,m.nombre as materia,m.foto, u.id as padron , u.nombre as nombProfesor, u.apellido apeProfesor, m.carga_horaria, m.descripcion, c.nombre as carrera from materias m
    join carreras c
    on c.id = m.carrera
    join usuario u
    on u.id =  m.profesor
