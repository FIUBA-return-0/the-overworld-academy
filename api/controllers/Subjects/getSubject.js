...
const getSubject = async (data) => {
  ...
  const query = `
    select m.id,m.nombre as materia,m.foto , u.nombre as nombProfesor, u.apellido apeProfesor, m.carga_horaria, c.nombre as carrera, m.descripcion from materias m
    join carreras c
    on c.id = m.carrera
    join usuario u
    on u.id =  m.profesor
